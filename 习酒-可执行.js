...
async function main() {
    ...
    for (const item of XiJiu) {
        ...
        console.log(token)
        console.log("模式打开首页小程序")
        let open = await openMiniProgram("api/open/event-log/E0", JSON.stringify({"vipId": login.data.member_id, "eventName": "进入首页页面"}));
        ...
        console.log("模式打开 习酒文旅")
        open = await openMiniProgram("api/open/event-log/E1", JSON.stringify({"vipId": login.data.member_id}));
        console.log(open.message)
        ...
        if (tasks) {
            const resultArray = Object.values(tasks.data);
...
async function openMiniProgram(url, body = '') {
    return new Promise(resolve => {
        const options = {
            url: `https://zhdl-dl.gzxijiu.com/${url}`,
            headers: {
                'Connection': 'keep-alive',
                'Authorization': token,
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/116.0.0.0 Safari/537.36 MicroMessenger/7.0.20.1781(0x6700143B) NetType/WIFI MiniProgramEnv/Windows WindowsWechat/WMPF',
    ...
    return new Promise(resolve => {
        const options = {
            ...
            headers: {
                ...
                'Authorization': token,
                // 'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/116.0.0.0 Safari/537.36 MicroMessenger/7.0.20.1781(0x6700143B) NetType/WIFI MiniProgramEnv/Windows WindowsWechat/WMPF',
                'User-Agent': 'Mozilla/5.0 (iPhone; CPU iPhone OS 17_4 like Mac OS X) AppleWebKit/605.1.15 (KHTML, like Gecko) Mobile/15E148 MicroMessenger/8.0.59(0x18003b2e) NetType/WIFI Language/zh_CN',
                'Content-Type': 'application/json',
                'Accept': '*/*',
                'Origin': 'https://mallwm.exijiu.com',
                'Sec-Fetch-Site': 'cross-site',
    ...
    return new class {
        ...
        writedata() {
            if (this.isNode()) {
                this.fs = this.fs ? this.fs : require("fs"), this.path = this.path ? this.path : require("path");
                const t = this.path.resolve(this.dataFile), e = this.path.resolve(process.cwd(), this.dataFile), s = this.fs.existsSync(t),
                    i = !s && this.fs.existsSync(e), o = JSON.stringify(this.data);
