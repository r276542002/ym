...
class Task {
    ...
    async main() {
        ...
        if (this.artList.length > 0) {
            for (let act of this.artList) {
                ...
                await this.task_comment(act)
                await this.task_share(act)
            }
        }
        for(let i=0;i<3;i++){
            await this.task_add(i)
        }
    }
    ...
    async task_sign() {
        ...
        //console.log(options);
        if(result.result['__showToast']){
            $.log(result.result['__showToast']['title']);
        }else{
            $.log('签到成功,+1积分！');
        }
    }
    ...
    async task_comment(id) {
        //评论帖子
        let result = await this.taskRequest("post", `https://ziwi.gzcrm.cn/json-rpc?__method=CommentThread`, {"id":1712040600068,"jsonrpc":"2.0","method":"CommentThread","params":{"threadId":id,"content":"11111","threadCommentId":0,"level":"info"}})
        
        $.log('评论成功，+2积分！');
    }
    async task_add(i) {
        //发布帖子
        let result = await this.taskRequest("post", `https://ziwi.gzcrm.cn/json-rpc?__method=AddThread`, {"id":Date.now(),"jsonrpc":"2.0","method":"AddThread","params":{"title":"用户帖子","topicId":"612832963310276608","content":"产品非常不错哦~" + i,"mediaFiles":[{"type":"image","path":"https://ziwi-cdn.gzcrm.cn/2023/07/06/069805b8558346ed92d5a12f23cee5a3.jpg","thumb":"https://ziwi-cdn.gzcrm.cn/2023/07/06/069805b8558346ed92d5a12f23cee5a3.jpg","size":0}],"level":"info"}})
